...
int main(void)
{
    ...
    while (1) {
    	...
    	USART_SendDataIT(&usart2_handle, (uint8_t*)msg[cnt], strlen(msg[cnt]));

    	//a quick fix to prevent first RX from garbage data
    	usart2_handle.pUSARTx->SR &= ~(1 << USART_SR_RXNE);
